...
func main() {
	...
	ans := AscIntSearch(0, pow(10, 9)+1, func(num int) bool {
		aIdx := sort.Search(len(As), func(i int) bool {
			return As[i] <= num
		})
		sellable := N - aIdx

		bIdx := sort.Search(len(Bs), func(i int) bool {
			return Bs[i] >= num
		})
		affordable := M - bIdx

		return sellable >= affordable
	})
