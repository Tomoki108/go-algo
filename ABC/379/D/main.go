...
func main() {
	...
	for i := 0; i < Q; i++ {
		...
		if sl[0] == 1 {
			plantHeights = append(plantHeights, -totalGrowth) // 後で「plantHeights[i]+totalGrowth >= h」で二分探索するので、恩恵を受けていないtotalGrowthをあらかじめ引いておく
		} else if sl[0] == 2 {
			t := sl[1]
			totalGrowth += t
		} else if sl[0] == 3 {
			...
			idx := sort.Search(len(plantHeights), func(i int) bool {
				return plantHeights[i]+totalGrowth >= h
			})
